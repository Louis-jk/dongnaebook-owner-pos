...
export default function OrderRejectModal(props: IProps) {
  ...
  const sendRejectHandler = () => {
    if (rejectValue === "") {
      ...
      let result;
      if (rejectValue !== "6") {
        let filtered = rejectInitState.filter(
          (item) => item.value === rejectValue
        );
        ...
        <Fade in={props.isOpen}>
          <Box className={clsx(base.modalInner, base.colCenter)}>
            <Typography
              ...
              variant="h5"
              mb={1}
              className={base.modalTitle}
            >
              주문 거부 사유 등록
            </Typography>
            <Typography id="transition-modal-description">
              주문 거부 사유를 선택 또는 입력해주세요.
            </Typography>
                ...
                <Grid key={index + item.label} item xs={6} md={4}>
                  <Button
                    ...
                    style={{
                      ...
                      padding: 10,
                      backgroundColor:
                        rejectValue === item.value
                          ? theme.palette.primary.main
                          : "#fff",
                      color: "#222",
                      borderColor:
                        rejectValue === item.value ? "#FCDD00" : "#e6e6e6",
                    }}
                    onClick={() => rejectHandler(item.value)}
                  >
                    {item.label}
                  </Button>
                </Grid>
            ...
            >
              <ModalConfirmButton
                ...
                onClick={sendRejectHandler}
              >
                보내기
              </ModalConfirmButton>
              <ModalCancelButton variant="outlined" onClick={handleCloseReject}>
                취소
              </ModalCancelButton>
            </ButtonGroup>
          </Box>
