...
export default function CloseStoreModal(props: IProps) {
  ...
  const setCloseStoreHandler = (
    ...
    const checked = allStore
      .filter((store: any) => store.mt_id === s_id)
      .filter((state: any) => state.do_end_state === "Y");
        ...
        <Box className={clsx(base.modalInner, base.colCenter)} p={5}>
          ...
          <Box maxHeight={300} overflow="scroll">
            {allStore && allStore.length > 0 ? (
              allStore.map((store: Object, index: number) => (
                <Box
                  key={index}
                  display="flex"
                  flexDirection="row"
                ...
                >
                  ...
                  <Box flexShrink={1} textAlign="left" style={{ minWidth: 70 }}>
                    <Typography
                      ...
                      style={{
                        borderRadius: 5,
                        backgroundColor:
                          store.do_end_state === "N" ? "#F8485E" : "#9FE6A0",
                  ...
                  <Box flexShrink={1}>
                    <Android12Switch
                      color="primary"
                      onChange={() =>
                        setCloseStoreHandler(
                          store.id,
                          store.mt_id,
                          store.mt_jumju_code
                        )
                      }
                      checked={store.do_end_state === "N" ? false : true}
                    />
                  </Box>
                </Box>
              ))
