...
export default function OrderCard(props: OrderProps) {
  ...
  const deliveryOrderCompleteHandler = (
    id: string,
    jumjuId: string,
    jumjuCode: string,
    type: string
    ...
    return orders.map((order, index) => (
      <Grid item xs={12} key={order.od_id + index}>
        ...
        <Box
          display="flex"
          flexDirection="row"
          justifyContent={type === "done" ? "flex-start" : "space-between"}
          alignItems="center"
          pb={0.5}
        >
          <Box
          ...
          >
            ...
            <Typography fontSize={34} fontWeight="bold" style={{ margin: 0 }}>
              {moment(order.od_time, "YYYY-MM-DD HH:mm:ss").format("HH:mm")}
            </Typography>
          </Box>
          <Box
            flex={type === "done" ? 2 : 7}
              ...
              <Box display="flex" flexDirection="row" mb={1}>
                <Typography
                  ...
                  color={
                    ...
                    order.od_settle_type === "card-face"
                      ? "red"
                      : "black"
                  }
                >
          ...
          >
            ...
            >
              <p>
                <img
                  src={order.image}
                  alt={`${order.mb_company}의 로고`}
                  title={`${order.mb_company}의 로고`}
                  width={40}
  ...
  return orders.length > 0 ? (
    <>
      <OrderCheckModal
        isOpen={open}
        od_id={odId}
        currJumjuId={currJumjuId}
        currJumjuCode={currJumjuCode}
        od_type={odType}
      ...
      <OrderDeliveryModal
        ...
        currJumjuCode={currJumjuCode}
        od_type={odType}
        handleClose={handleDeliveryClose}
      />
      <OrderDeliveryCompleteModal
        isOpen={deliveryCompleteOpen}
        od_id={odId}
        currJumjuId={currJumjuId}
        currJumjuCode={currJumjuCode}
        od_type={odType}
        handleClose={handleDeliveryCompleteClose}
      />
      <OrderCancelModal
        ...
        currJumjuId={currJumjuId}
        currJumjuCode={currJumjuCode}
        handleClose={cancelModalCloseHandler}
      />
      <Box>{renderList()}</Box>
    </>
  ) : (
