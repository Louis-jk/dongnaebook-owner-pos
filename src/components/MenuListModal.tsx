...
const MenuListModal: React.FC<Props> = ({
  open,
  onClose,
  reflesh,
  propModalToastAction,
}) => {
    ...
    <>
      {menus && menus.length > 0 ? (
        ...
        >
          <Fade in={open}>
            <Box
              className={base.modalInner}
              style={{ backgroundColor: "#25233e", width: 500 }}
            >
              <Box mb={2}>
                <Typography
                  component="h2"
                        ...
                        >
                          {menu.it_img1 ? (
                            <img
                              ...
                              alt={`${menu.ca_name}의 썸네일`}
                              title={`${menu.ca_name}의 썸네일`}
                            />
                          ) : (
                            <Box
                              style={{
                          ...
                          >
                            <Box className={menuSt.menuListTitle}>
                              ...
                              {menu.it_type1 === "1" ? (
                                <span className={menuSt.menuListLabel01}>
                                  대표메뉴
                                </span>
                              ) : null}
                              {menu.it_use === "1" ? (
                                <span className={menuSt.menuListLabel02}>
                                  판매중
                                </span>
                              ) : (
                                <span className={menuSt.menuListLabel03}>
                                  판매중지
                                </span>
                              )}
                              {menu.it_soldout === "1" ? (
                                <span className={menuSt.menuListLabel05}>
                                  품절
                                </span>
                              ) : null}
                            </Box>
              ...
              >
                ...
                <ModalCancelButton variant="outlined" onClick={onClose}>
                  닫기
                </ModalCancelButton>
              </ButtonGroup>
            </Box>
